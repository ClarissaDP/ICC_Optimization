...
void gaussSeidel(int nx, int ny, int maxiter, FILE *saida) {
    int i, j, iter;
    double fx[nx], fy[ny], mulPI, grade;
    ...
    if(!( B = (double**)malloc( ny *sizeof(double)) )) {
        fprintf(stderr, "Erro na alocação da matriz B.\n");
        return;
    }   
    for(i = 0 ; i < nx ; ++i)
        if(!( B[i] = (double*)malloc( nx *sizeof(double)) )) {
          fprintf(stderr, "Erro na alocação da matriz B.\n");
          return;
        }   
    ...
    for(iter = 0; iter < maxiter; ++iter) {
        ...
        for(i = 1; i < (nx+1) ; ++i) {
            for(j = 1; j < (ny+1) ; ++j){
            		/*metodo*/
            		time = timestamp();
            			X[i][j] = grade *( (mulPI*fy[ny-i]*fx[j-1]) + hxD*(X[i-1][j] + X[i+1][j]) + hyD*(X[i][j-1] + X[i][j+1]));
