        ...
        for(i = 1; i <= ny ; ++i) {
            ...
            for(; j <= nx ; j+=8){
              ...
              /*metodo*/
            		...
            		X[i * nx + j + 3] = aux3;
//----------------------------------
								aux1 = hxD*(X[(i-1) * nx +j + 4] + X[(i+1)*nx + j + 4]) + hyD*(aux3 + X[i*nx +j+5]);
            		aux3 = grade *( B[(i-1) * nx + j + 3] + aux1);
            		X[i * nx + j + 4] = aux3;
            		
            		aux1 = hxD*(X[(i-1) * nx +j + 5] + X[(i+1)*nx + j + 5]) + hyD*(aux3 + X[i*nx +j+6]);
            		aux3 = grade *( B[(i-1) * nx + j + 4] + aux1);
            		X[i * nx + j + 5] = aux3;
            		
            		aux1 = hxD*(X[(i-1) * nx +j + 6] + X[(i+1)*nx + j + 6]) + hyD*(aux3 + X[i*nx +j+7]);
            		aux3 = grade *( B[(i-1) * nx + j + 5] + aux1);	
            		X[i * nx + j + 6] = aux3;
            		
            		aux1 = hxD*(X[(i-1) * nx +j + 7] + X[(i+1)*nx + j + 7]) + hyD*(aux3 + X[i*nx +j+8]);
            		aux3 = grade *( B[(i-1) * nx + j + 6] + aux1);
            		X[i * nx + j + 7] = aux3;
            ...
            for(; j <= nx ; j+=8){
              ...
              /*residuo*/
                time = timestamp();
                	...
                	aux4 = X[i*nx + j + 2];
//----------------------------
									residuo[iter] += fabs(  - ( aux2 * aux1 - (hxD*(X[(i-1) * nx +j + 4] + X[(i+1)*nx + j + 4]) + hyD*(aux3 + aux4))));
                	aux3 = aux1;
                	aux1 = aux4;
                	aux4 = X[i*nx + j + 2];
                	
                	residuo[iter] += fabs(  - ( aux2 * aux1 - (hxD*(X[(i-1) * nx +j + 5] + X[(i+1)*nx + j + 5]) + hyD*(aux3 + aux4))));
                	aux3 = aux1;
                	aux1 = aux4;
                	aux4 = X[i*nx + j + 2];
                	
                	residuo[iter] += fabs(  - ( aux2 * aux1 - (hxD*(X[(i-1) * nx +j + 6] + X[(i+1)*nx + j + 6]) + hyD*(aux3 + aux4))));
                	aux3 = aux1;
                	aux1 = aux4;
                	aux4 = X[i*nx + j + 2];
                	
                	residuo[iter] += fabs(  - ( aux2 * aux1 - (hxD*(X[(i-1) * nx +j + 7] + X[(i+1)*nx + j + 7]) + hyD*(aux3 + aux4))));
                	aux3 = aux1;
                	aux1 = aux4;
                	aux4 = X[i*nx + j + 2];
