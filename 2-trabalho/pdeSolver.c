...
void inicializa(double *X, double *B, int ny, int nx, double *fx, double *fy, double hx, double hy, double *residuo) {
    int i = 0,k, j;
    ...
    for(j = 1 ; j < (nx+1); ++j) {
      X[i*nx + j] = sin(2.0*3.1415 * j * hx) * superior;
    }
    ...
    for( i=0; i < ny; ++i ) 
    		for(j = 0; j < nx; ++j)
    			B[i*nx + j] = mul_PI * fy[ny -i] * fx[j];
    
    for (k = 0; k < maxIter; ++k){			
    	residuo[k] = 0.0;
		  for( i=0; i < ny; ++i ) 
		  		for(j = 0; j < nx; ++j)
				  	residuo[k] += fabs(B[i*nx + j]);
		}
    return;
}
